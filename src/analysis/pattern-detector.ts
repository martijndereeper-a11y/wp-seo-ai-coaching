...
// Tri-lingual pattern definitions (NL | DE | EN).
// Each regex unions Dutch, German, and English tokens so behavior detection
// works across all three markets. Adding a new language = adding a third union branch.
const PATTERN_DEFS: Record<keyof PatternScores, RegExp> = {
  marketContext: /zoekgedrag|zoekmarkt|seo.*verandert|online.*vindbaar|suchverhalten|suchmarkt|seo.*ändert|online.*auffindbar|search.*behavior|search.*market|seo.*chang|online.*findab|google|chatgpt|ai overviews|perplexity|gemini/gi,
  contentEngine: /content|blog|artik|publicer|schrijven|tekst|pagina.*per.*maand|inhalt|veröffentlich|schreiben|seite.*pro.*monat|publish|write|page.*per.*month/gi,
  visibility: /vindbaar|zichtbaar|gevonden.*worden|zoekresultat|ranking|positie|organisch|auffindbar|sichtbar|gefunden.*werden|suchergebnis|organisch|findable|visible|search.*result|be.*found|organic/gi,
  dataDriven: /data|analys|meten|dashboard|rapportage|inzicht|tracking|monitor|analyse|messen|bericht|einblick|measurement|report|insight/gi,
  companyIntro: /wij zijn|we zijn|opgericht|kantoor|medewerkers|ons team|amsterdam|finland|wir sind|gegründet|büro|mitarbeiter|unser team|niederlande|finnland|we are|we're|founded|office|employees|our team|netherlands/gi,
  aiAngle: /\bai\b|ai.gedreven|kunstmatige|artificial|\bki\b|künstliche.*intelligenz|machine.*learning|llm|large.*language/gi,
  authority: /expert|specialist|marktleider|bewezen|track.record|professionals|spezialist|marktführer|bewährt|nachweislich|market.*leader|proven|track.*record/gi,
  socialProof: /\d+.*klanten|\d+.*bedrijven|2000|tweeduizend|duizend.*klant|\d+.*kunden|\d+.*unternehmen|tausend.*kunden|\d+.*customers|\d+.*companies|thousand.*customer|two.*thousand/gi,
  aspiration: /marktleider|groeien|schalen|volgende.*stap|ambitie|potentieel|mogelijkheid|kansen|marktführer|wachsen|skalieren|nächste.*schritt|ambition|potenzial|möglichkeit|chance|market.*leader|grow|scale|next.*step|potential|opportunit/gi,
  vsAgency: /bureau|agency|freelanc|extern.*inhuur|traditioneel|agentur|extern.*einstell|traditionell|external.*hire|traditional/gi,
  compounding: /compound|stapel|sneeuwbal|snowball|exponenti|over.*tijd.*groei|stapeln|schneeball|exponentiell|über.*zeit.*wachs|over.*time.*grow/gi,
  activeListening: /als.*ik.*het.*goed.*begrijp|dus.*je.*zegt|interessant|goed.*punt|snap.*ik|herkenbaar|wenn.*ich.*richtig.*verstehe|also.*sagen.*sie|guter.*punkt|verstehe|if.*i.*understand.*correctly|so.*you.*'re.*saying|so.*you.*are.*saying|good.*point|makes.*sense/gi,
  roiReframe: /investering|verdien.*terug|roi|terugverdien|waarde.*versus|investition|amortisier|verdienen.*zurück|wert.*gegenüber|investment|pay.*back|payback|value.*versus/gi,
  urgency: /snel|direct|meteen|zo snel mogelijk|nu.*het.*moment|wachten.*kost|schnell|sofort|so.*schnell.*wie.*möglich|warten.*kostet|quickly|immediately|right.*now|asap|waiting.*cost/gi,
  contract: /contract|onderteken|sign|overeenkomst|akkoord|bevestig|vertrag|unterzeichn|unterschreib|vereinbarung|bestätig|zustimm|agreement|confirm/gi,
  personalStory: /wij hadden|ik had|bij ons|wij merkten|wij zagen|onze ervaring|wir hatten|ich hatte|bei uns|wir bemerkten|wir sahen|unsere erfahrung|we had|i had|in our case|we noticed|we saw|our experience/gi,
  pricing: /prijs|pricing|budget|offerte|kosten|pakket|tarief|investering|€|\d+.*euro|preis|angebot|paket|tarif|investition|price|cost|package|tier|investment|\$\d/gi,
  humor: /haha|grap|lach|grappig|leuk|funny|witz|lustig|spaßig|joke|laugh/gi,
  theirBusiness: /jullie.*klant|jullie.*markt|jullie.*concurrent|jullie.*product|jullie.*dienst|jullie.*omzet|jullie.*groei|ihre.*kunde|ihre.*markt|ihre.*konkurrent|ihr.*produkt|ihre.*dienst|ihre.*umsatz|ihr.*wachstum|your.*customer|your.*market|your.*competitor|your.*product|your.*service|your.*revenue|your.*growth/gi,
  checkIn: /gaat.*te snel|nog.*mee|duidelijk|alles.*helder|vragen.*tot.*nu|tot zover|zu.*schnell|noch.*dabei|alles.*klar|alles.*verständlich|fragen.*bis.*jetzt|bis.*hierher|stimmt das|too.*fast|still.*with.*me|any.*question.*so.*far|so.*far.*so.*good|does.*that.*make.*sense/gi,
  opinionAsk: /wat denk je|wat vind je|hoe klinkt|hoe.*kijk.*je|wat.*zegt.*gevoel|wat.*gaat.*door.*hoofd|was.*denken.*sie|wie.*klingt|wie.*sehen.*sie|was.*sagt.*ihr.*gefühl|what.*do.*you.*think|how.*does.*that.*sound|how.*do.*you.*see|what.*'s.*your.*gut|what.*'s.*going.*through.*your.*mind/gi,
  research: /ik.*zag.*op.*jullie|ik.*keek.*naar|ik.*heb.*gekeken|jullie.*website|jullie.*linkedin|jullie.*google|ich.*sah.*auf.*ihre|ich.*schaute.*nach|ich.*habe.*mir.*angeschaut|ihre.*website|ihr.*linkedin|i.*saw.*on.*your|i.*looked.*at|i.*checked|your.*website|your.*linkedin/gi,
  directness: /wat wij doen|wat we doen|hoe het werkt|in een notendop|simpel gezegd|kort gezegd|even recht.*door|eerlijk|transparant|was.*wir.*machen|was.*wir.*tun|wie.*es.*funktioniert|kurz.*gesagt|einfach.*gesagt|ehrlich.*gesagt|transparent|what.*we.*do|how.*it.*works|in.*a.*nutshell|simply.*put|honestly|transparent/gi,
  priceAnchor: /bureau.*kost|normaal.*kost|gemiddeld|vergelijk|als.*je.*kijkt.*naar|wat.*je.*nu.*betaalt|bespaart|agentur.*kostet|normalerweise.*kostet|durchschnittlich|vergleich|was.*sie.*jetzt.*zahl|spar|agency.*cost|normally.*cost|on.*average|compar|what.*you.*currently.*pay|save/gi,
  assumptiveClose: /wanneer.*starten|welk.*pakket|als we.*beginnen|als we.*starten|ik stuur.*contract|volgende stap.*is|wann.*anfangen|welches.*paket|wenn.*wir.*beginnen|wenn.*wir.*starten|ich.*schicke.*vertrag|nächste.*schritt.*ist|when.*would.*you.*start|which.*package|if.*we.*start|i.*'ll.*send.*the.*contract|i.*will.*send.*contract|next.*step.*is/gi,
  challenging: /maar.*dan|waarom.*niet|wat.*houdt.*tegen|wat.*weerhoudt|als.*niet.*dan|stel.*dat|aber.*dann|warum.*nicht|was.*hält.*auf|was.*hindert|wenn.*nicht.*dann|angenommen|but.*then|why.*not|what.*stops.*you|what.*holds.*you|if.*not.*then|suppose.*that/gi,
};

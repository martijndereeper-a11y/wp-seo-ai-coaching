...
// Game leaderboard — all AEs ranked by avg game score
...
// AE game history — individual AE's game scores over time
app.get('/api/game/ae/:name', async (c) => {
  ...
  return c.json({
    ...
    calls: (data || []).map(d => ({
      ...
      outcome: d.outcome,
      date: d.created_at,
      gameScore: d.game_score,
    })),
  });
});

// Live game score — score a call on-the-fly (for calls not yet analyzed)
app.get('/api/call/:id/game-score', async (c) => {
  const id = c.req.param('id');

  // First check if we already have a stored score
  ...
  if (analysis?.game_score?.totalPoints !== undefined) {
    return c.json(analysis.game_score);
  }

  // Otherwise compute from transcript
  const recording = await fetchParsedTranscript(id);
